...
export async function createProductAction(input: unknown): Promise<ActionResult<Product>> {
  ...
  if (!parsed.success) {
    return { ok: false, message: "Invalid product data", details: parsed.error.issues.map((issue) => issue.message) };
  }
...
export async function updateProductAction(productId: string, input: unknown): Promise<ActionResult<Product>> {
  const parsed = productFormSchema.safeParse(input);
  if (!parsed.success) {
    return { ok: false, message: "Invalid product data", details: parsed.error.issues.map((issue) => issue.message) };
  }
...
export async function createCategoryAction(input: unknown): Promise<ActionResult<Category>> {
  ...
  if (!parsed.success) {
    return { ok: false, message: "Invalid category data", details: parsed.error.issues.map((issue) => issue.message) };
  }
...
export async function updateCategoryAction(categoryId: string, input: unknown): Promise<ActionResult<Category>> {
